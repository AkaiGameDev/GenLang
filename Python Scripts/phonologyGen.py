...
class IpaSound:
    def __init__(self, descriptive_name, ipa_char):
        self.descriptive_name = descriptive_name
        self.ipa_char = ipa_char

    def __str__(self):
        return self.ipa_char
...
class IpaConsonant(IpaSound):
    def __init__(self, descriptive_name, ipa_char, commonness, phonation, manner, place, sound_type):
        super().__init__(descriptive_name, ipa_char)
    ...
    def __repr__(self):
        return f'This sound\'s IPA character is {self.ipa_char}, its descriptive_name is {self.descriptive_name}, ' \
               f'\nits place of articulation is {self.place}, and its manner of articulation is {self.manner}'


class IpaVowel(IpaSound):
    def __init__(self, descriptive_name, ipa_char, height, roundedness, backness):
        super().__init__(descriptive_name, ipa_char)
        self.height = height
...
def create_3_vowel_system():
    ...
    output = []
    for v in allvowels:
        if v.ipa_char == 'i' or v.ipa_char == 'u' or v.ipa_char == 'a':
            output.append(v)
    return output
...
def create_5_vowel_system():
    ...
    output = []
    for v in all_vowels:
        if v.ipa_char == 'i' or v.ipa_char == 'u' or v.ipa_char == 'e' or v.ipa_char == 'o' or v.ipa_char == 'a':
            output.append(v)
    return output
...
def select_consonants(constraints=None):
    ...
    for constraint in constraints:
        if 'no:' in constraint:
            ...
            for consonant in working_consonants:
                if to_be_removed == consonant.ipa_char or to_be_removed == consonant.descriptive_name:
    ...
    for constraint in constraints:
        if 'has:' in constraint:
            ...
            for consonant in working_consonants:
                if to_be_added == consonant.ipa_char or to_be_added == consonant.descriptive_name:
    ...
    if constraints is not None:
        for c in constraints:
            if 'vowel inventory size:' in c:
                ...
                for v in all_vowels:
                    if v.ipa_char == to_be_added or v.descriptive_name == to_be_added:
                        selected_vowels.append(v)
            elif 'no:' in c:
                to_be_removed = c.replace('no:', '').strip()
                for v in all_vowels:
                    if v.ipa_char == to_be_removed or v.descriptive_name == to_be_removed:
                        all_vowels.remove(v)
    ...
    for c in consonants:
        if c.sound_type == 'pulmonic':
            ...
            if c.phonation == 'voiceless':
                pulmonic_consonants[row][column][0] = c.ipa_char
            elif c.phonation == 'voiced':
                pulmonic_consonants[row][column][1] = c.ipa_char
            else:
                print(f'{c.descriptive_name} has a phonation that is not set correctly')
        ...
        if c.sound_type == 'nonpulmonic':
            ...
            if 'ejective' in c.manner:
                ...
                # this is out of order because fricative is in lateralfricative
                    ...
                    print(f'error finding row for {c.ipa_char}')
                    exit()
            ...
            if 'lateralclick' in c.manner:
                ...
                print(f'error finding column for {c.ipa_char}')
                exit()

            output[row][column] = c.ipa_char
